...
if(mywin1_W < mywin1_H){
  ...
  var width = 0.70*mywin1.innerWidth - margin.left - margin.right;
  var height = 0.90*mywin1.innerHeight - margin.top - margin.bottom;
}
...
fetch('https://m3jxkh.deta.dev/analystratings/symbolnames',
        {
            method: "GET",
    ...
    .then(data => {
        ...
        fetch('https://m3jxkh.deta.dev/analystratings/ratings?symbol_name='+selectedTicker,
                {
                    method: "GET",
                    //body: JSON.stringify(data),
                    mode: 'cors',
...
*/
                ...
                if(!mobile_mode){
                  ...
                  var x = d3.scaleLinear()
                    .domain(d3.extent(dataFilter, function(d) { return parseTime(d.date); }))
                    //.domain([0,10])
                    .range([ 0, width ]);
                  svg.append("g")
              ...
              // Add X axis --> it is a date format
                  ...
                  svg.append("g")
                    ...
                    .call(xAxis
                      .tickFormat((d, i) => {
                          ...
                          if ( i > 0 && ((new Date(d)).getMonth() == 0 )  && ((new Date(d)).getMonth() != (new Date(ticks[i-1])).getMonth() )) {
                  ...
                  svg.append("text")
                      .style("font", 1.5*chart_font.size + "px " + chart_font.font)
                      .attr("transform", "rotate(90)")
                      .attr("y", -width - 1.00*margin.right)
                      .attr("x",0 + (height / 2))
                      .attr("dy", "1em")
                      .style("text-anchor", "middle")
                      .style("fill", csvColors[3])
                ...
                function update() {
                  ...
                  for(i = 0; i<4; i++){
                    var dataPlotted = dataFilter.map(function(d){return {date: parseTime(d.date), value:d[allLines[i]]} })
                  ...
                  //line
                    all_lines[i]
                        ...
                        .attr("d", d3.line()
                          ...
                          .y(function(d) { return all_yaxes[i]((+d.value)*all_ymults[i]) })
                        )
                    ...
                    svg.selectAll("myCircles")
                      ...
                      .attr("cy", function(d) { return all_yaxes[i]((+d.value)*all_ymults[i]) })

                  //debugger;
                  }

                  // Add Yright axis
                  y1Axis.transition().duration(1000).call(d3.axisRight(y1).ticks(5).tickSize(0))
